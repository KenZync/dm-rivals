...
function OJNParser(hitSounds) {
  ...
  var parseOJN = function (buffer) {
    ...
    const title_raw = Buffer.from(buffer, 108, 64);
    data.header.title = iconv.decode(title_raw, "CP949").replace(/\0/g, "");

    const artist_raw = Buffer.from(buffer, 172, 32);
    data.header.artist = iconv.decode(artist_raw, "CP949").replace(/\0/g, "");

    const noter_raw = Buffer.from(buffer, 204, 32);
    data.header.noter = iconv.decode(noter_raw, "CP949").replace(/\0/g, "");

    const ojm_file_raw = Buffer.from(buffer, 236, 32);
    data.header.ojmFile =  iconv.decode(ojm_file_raw, "CP949").replace(/\0/g, "");
    ...
    const cover_offset_end =  data.header.coverOffset + data.header.coverSize;
    const bmp_offset_end = cover_offset_end + data.header.bmpSize;
    
    const cover_base64 = Buffer.from(
      buffer.slice(data.header.coverOffset, cover_offset_end)
      ).toString("base64");
      const cover = "data:image/png;base64," + cover_base64;
    const bmp_base64 = Buffer.from(
      buffer.slice(cover_offset_end, bmp_offset_end)
    ).toString("base64");
