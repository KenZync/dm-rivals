<template>
    <div class="min-h-screen bg-zinc-900">
        <div class="container mx-auto">
            <div class="pt-8 text-center text-5xl text-stone-200">DMJam Rival System</div>
            <form>
                <div class="flex space-x-2">
                    <div>
                        <label for="compare1" class="text-md font-medium text-stone-200">User 1</label>
                        <div class="mt-1">
                            <input type="text" v-model="user1" id="compare1" name="compare2" placeholder="Name 1"
                                autofocus
                    ...
                    <div>
                        <label for="compare2" class="text-md font-medium text-stone-200">User 2</label>
                        <div class="mt-1">
                            <input type="text" v-model="user2" id="compare2" name="compare2" placeholder="Name 2"
                                class="text-stone-200 bg-zinc-800 rounded-md border-gray-600 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" />
                        </div>
                    </div>
                    <div>
                        ...
                        <div v-if="fetching"
                            ...
                            <div class="animate-spin inline-block w-5 h-5 border-[3px] border-current border-t-transparent text-blue-600 rounded-full"
                                role="status" aria-label="loading">
                                <span class="sr-only">Loading...</span>
                            </div>
                            <div class="pl-2">Loading</div>
                        </div>
                        <div v-else><button @click="getCompare"
            ...
            <fieldset class="space-y-5" v-if="compare">
                ...
                <div class="relative flex items-start">
                    ...
                    <div class="ml-3 text-sm">
                        <label for="comments" class="font-medium text-stone-200">Show only both Played</label>
                    </div>
                </div>
            </fieldset>
            ...
            <button
                ...
                v-if="compare" @click="toggleSortTime()">
            ...
            <div class="flex space-x-2" v-if="compare">
                <div>
                    <div class="text-center mb-2 text-stone-200 font-medium">{{ submitted1 }}'s Win</div>
                    <table class="min-w-full border border-gray-300 divide-y divide-gray-300 text-stone-200 table-auto">
                        ...
                        <tbody class="divide-y divide-gray-200">
                            <tr class="text-center" v-for="(score, key) in filteredCompareData1"
                                :class="key % 2 === 0 ? undefined : 'bg-zinc-800'">
                                <td>{{ score.Rank }}</td>
                                <td><a class="hover:underline hover:text-blue-500"
                                        :href='("https://dpjam.net/music-scoreboard/" + score.ID)'>{{ score.Title }}</a>
                                </td>
                                <td>{{ score.Acc }}</td>
                                <td>{{ score.Progress }}</td>
                                <td><span
                                        class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium text-stone-100"
                                        :class="score.Clear == 'Cleared' ? 'bg-green-800' : 'bg-red-500'">{{ score.Clear
                                        }}</span>
                                </td>
                                <td>{{ score.Level }}</td>
                ...
                <div>
                    <div class="text-center mb-2 text-stone-200 font-medium">{{ submitted2 }}'s Win</div>
                    <table class="min-w-full border border-gray-300 divide-y divide-gray-300 text-stone-200 table-auto">
                        ...
                        <tbody class="divide-y divide-gray-200">
                            <tr class="text-center" v-for="(score, key) in filteredCompareData2"
                                :class="key % 2 === 0 ? undefined : 'bg-zinc-800'">
                                <td>{{ score.Rank }}</td>
                                <td><a class="hover:underline hover:text-blue-500"
                                        :href='("https://dpjam.net/music-scoreboard/" + score.ID)'>{{ score.Title }}</a>
...
useHead({
    ...
    meta: [
        { name: 'description', content: 'O2Jam score comparison' }
    ],
})
...
const getCompare = async () => {
    fetching.value = true;
    const data = await $fetch(`/api/compare?user1=${encodeURIComponent(user1.value)}&user2=${encodeURIComponent(user2.value)}`).catch(() => alert("ERROR TRY AGAIN")).finally(() => fetching.value = false)
